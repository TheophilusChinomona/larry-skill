 ...
 *   - gemini      (Gemini 2.0 Flash image generation — no OpenAI key needed)
 ...
 * Then install the SDK: npm install @google/generative-ai
 */
...
// Gemini outputs square images, so we center-crop to 1024×1536 portrait
// using node-canvas (already a Larry dependency for text overlays).
//
// Requirements:
//   npm install @google/generative-ai
//   Get a free API key at: https://aistudio.google.com/apikey
//
// Recommended model: gemini-2.0-flash-preview-image-generation
//
async function generateGemini(prompt, outPath) {
  ...
  try {
    ...
    throw new Error(
      'Missing dependency. Run: npm install @google/generative-ai\n' +
      'Then get a free API key at: https://aistudio.google.com/apikey'
    );
  }

  const genAI = new GoogleGenerativeAI(apiKey);
  const geminiModelName = model || 'gemini-2.0-flash-preview-image-generation';
  const genModel = genAI.getGenerativeModel({ model: geminiModelName });

  // Ask for portrait composition in the prompt — Gemini respects this even
  // though the output is square; center-crop then recovers correct framing.
  const portraitPrompt =
    `${prompt}\n\n` +
    `IMPORTANT: Compose this image for a vertical phone screen (9:16 portrait). ` +
    `Keep all important subjects centred vertically. Realistic photo style, ` +
    `iPhone photo, natural lighting.`;

  const result = await genModel.generateContent({
    contents: [{ role: 'user', parts: [{ text: portraitPrompt }] }],
    generationConfig: {
  ...
  if (!imagePart) {
    ...
    throw new Error(
      ...
      `Model response: ${textPart?.text ?? '(empty)'}\n` +
      `Check your API key and that the model supports image generation.`
    );
  }

  const rawBuffer = Buffer.from(imagePart.inlineData.data, 'base64');

  // ── Resize to 1024×1536 portrait (TikTok fullscreen) ──────────────
  // Gemini outputs square (typically 1024×1024). We scale to cover the
  // portrait canvas and center-crop — same as "cover" in CSS background-size.
  // canvas is already a required Larry dependency (used for text overlays).
  ...
  // Black fill for any letterbox edges (rare with cover-scale)
  ctx.fillStyle = '#000000';
  ctx.fillRect(0, 0, TARGET_W, TARGET_H);

  // Scale so the image covers the full portrait canvas, then centre
...
async function generate(prompt, outPath) {
  ...
  console.log(`  Generating ${path.basename(outPath)} [${provider}/${model || geminiDefault()}]...`);
  await withRetry(() => fn(prompt, outPath));
  console.log(`  ✅ ${path.basename(outPath)}`);
}

function geminiDefault() {
  return provider === 'gemini' ? 'gemini-2.0-flash-preview-image-generation' : model;
}

(async () => {
  const displayModel = model || (provider === 'gemini' ? 'gemini-2.0-flash-preview-image-generation' : 'default');
  console.log(`🎬 Generating 6 slides for ${config.app?.name || 'app'} using ${provider}/${displayModel}\n`);
  let success = 0;
  let skipped = 0;
  for (let i = 0; i < 6; i++) {
